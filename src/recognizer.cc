...
Recognizer::Recognizer(SpkModel *spk_model) : model_(0), spk_model_(spk_model) {
...
Recognizer::Recognizer(SpkModel *spk_model, bool need_mic) : model_(0), spk_model_(spk_model){
...
bool Recognizer::GetSpkVectorVad(Vector<BaseFloat> &out_xvector, int *num_spk_frames)
{
	//spk_feature_->
...
bool Recognizer::GetSpkVectorVadMic(Vector<BaseFloat> &out_xvector, int *num_spk_frames, float rec_len)
{
...
const char *Recognizer::XvectorResult()
{
	spk_feature_->InputFinished();
	json::JSON obj;
	stringstream text;
...
const char *Recognizer::GetSpksList(const char* path)
{
...
bool Recognizer::DeleteSpeaker(const char* path, const char* user_id)
{
...
const char *Recognizer::GetIdentityMic(const char* path, float rec_len, float& top_score)
{
	std::string out("");
